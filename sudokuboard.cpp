#include "sudokuboard.h"


SudokuBoard::SudokuBoard(QWidget *parent) : QWidget(parent)
{
    resize(420, 420);
    _blankGridNum=15;
    _sudoku = new Sudoku();
    setEnabled(false);
    setToolTip("请点击开始游戏，开始挑战");
    _glayout=NULL;
    init();
}

SudokuBoard::~SudokuBoard()
{
    delete _sudoku;
}

void SudokuBoard::setLevel(SudokuBoard::Level level)
{
    switch (level) {
    case Easy:
        _blankGridNum = 20;
        break;
    case Middle:
        _blankGridNum = 30;
        break;
    case Hard:
        _blankGridNum = 45;
        break;
    case Master:
        _blankGridNum = 60;
        break;
    case SuperMaster:
        _blankGridNum = 64;
        break;
    default:
        _blankGridNum = 15;
        break;
    }
}

void SudokuBoard::clearAnswer()
{
    _sudoku->copyArray(_raw_data,_going_data);
    updateItems();
}

bool SudokuBoard::checkAnswer()
{
    int index=0;
    foreach(ItemWidget *w,_items){
        if(!w){
            ++index;
            continue;
        }
        _going_data[index / 9][index % 9] = w->text().toInt();
        ++index;
    }
    return _sudoku->checkSolution(_going_data);
}

void SudokuBoard::resetData()
{
    initData();
    updateItems();
}

void SudokuBoard::updateItems()
{
    int index=0;
    foreach(ItemWidget *w,_items){
        if(!w){
            ++index;
            continue;
        }
        w->setValue(_going_data[index/9][index%9],true);
        ++index;
    }
}

void SudokuBoard::init()
{
    initData();
    _items.clear();
    _groups.clear();
    //栅栏布局，用来设定9个GroupBox的方格矩阵布局
    _glayout = new QGridLayout;
    for(int i=0;i<9;i++){
        QGroupBox *group= new QGroupBox;
        //栅栏布局，用来设定一个GroupBox控件的布局
        QGridLayout *layout = new QGridLayout;
        layout->setMargin(0);       //设置外边距
        group->setLayout(layout);   //GroupBox空间中设置栅栏布局
        //向外层栅栏布局中添加GroupBox控件
        _glayout->addWidget(group,i/3*3,i%3 * 3,3,3);
        _groups.append(group);      //将控件添加到数组中
    }
    _glayout->setSpacing(0);

    for(int i=0;i<9;++i){
        for(int j=0;j<9;++j){
            ItemWidget * w = new ItemWidget(_going_data[i][j]);
            _items.append(w);
            int pos=i/3*3+j/3;  //计算所属3x3矩阵在group中的索引
            QGridLayout *layout=qobject_cast<QGridLayout*>(_groups[pos]->layout());
            if(!layout){
                qDebug()<<"internal error!";
                continue;
            }
            layout->addWidget(w,i%3,j%3);
        }
    }
    setLayout(_glayout);
}

void SudokuBoard::initData()
{
    _sudoku->makeLevel(_raw_data,_blankGridNum);
    _sudoku->copyArray(_raw_data,_going_data);
}

void SudokuBoard::paintEvent(QPaintEvent *e)
{
    Q_UNUSED(e);
}
